...
type Router struct {
	...
	ForceHashURL bool
	// Verbose determines whether or not the router will fmt to console.fmt.
	// If true, the router will fmt a message if, e.g., a match cannot be found for
	// a particular path.
	Verbose bool
...
func (r *Router) pathChanged(path string, initial bool) {
	...
	if bestRoute == nil {
		if r.Verbose {
			fmt.Println("Could not find route to match: " + path)
		}
		return
	}
...
func (r Router) parseQueryPart(queryPart string) (params map[string][]string) {
	...
	if err != nil && r.Verbose {
		...
		// part of the URL, so we optionally fmt a message
		fmt.Printf("Error parsing query %v: %v", queryPart, err)
	}
	return
}
